...
class PipelineDetection_v1_0():
    def __init__(self, path_cfg=None, mode='train'):
        ...
        *   'test'  denotes mode for inference
        ...
        self.mode = mode
        self.split_summary = verify_split_contract(self.cfg)
        if self.split_summary.get('enabled', False):
            print(f"* {format_split_summary(self.split_summary)}")
        ...
        clean_training = self.mode == 'train' and bool(
            self.cfg.VAL.get('USE_CLEAN_EVALUATOR', False)
        )
        self.dataset_train = build_dataset(self, split='train') if self.mode == 'train' else None
        if (
            self.mode == 'train'
            and bool(self.cfg.VAL.get('IS_VALIDATE', False))
            and self.cfg.DATASET.PATH_SPLIT.get('val', None)
        ):
            self.dataset_val = build_dataset(self, split='val')
        else:
            self.dataset_val = None
        self.dataset_test = None if clean_training else build_dataset(self, split='test')
        if mode == 'train':
        ...
        # Validation/evaluation runtime state.  ``IS_VALIDATE`` controls
        # training-time validation only; an explicit test-mode pipeline must
        # always initialize the evaluator fields even when a refit config has
        # training-time validation disabled.
        if self.cfg.VAL.IS_VALIDATE or self.mode == 'test':
            if self.is_distributed:
                if self.local_rank == 0:
                    self.set_validate()
                else:
    ...
    def set_validate(self):
        ...
        self.use_clean_evaluator = bool(self.cfg.VAL.get('USE_CLEAN_EVALUATOR', False))
        self.eval_split_during_training = str(self.cfg.VAL.get('EVAL_SPLIT_DURING_TRAINING', 'val'))
        self.fail_on_inference_error = bool(self.cfg.VAL.get('FAIL_ON_INFERENCE_ERROR', True))
        self.ap_mode = str(self.cfg.VAL.get('AP_MODE', 'R11')).upper()
        if self.use_clean_evaluator and self.ap_mode != 'R11':
            raise ValueError('The aligned 3D-LRF evaluator currently supports AP_MODE=R11 only')
        self.model_selection_iou = float(self.cfg.VAL.get('MODEL_SELECTION_IOU', 0.3))
        self.model_selection_class = str(self.cfg.VAL.get('MODEL_SELECTION_CLASS', 'sed'))
    ...
    def _assert_stage1_frozen(self):
        if not bool(self.cfg.MODEL.IMG_CLS.get('FREEZE_ALL_IN_STAGE2', False)):
            return
        model = self.network.module if self.is_distributed else self.network
        image_encoder = model.img_cls
        trainable = [name for name, parameter in image_encoder.named_parameters() if parameter.requires_grad]
        training_bn = [
            name for name, module in image_encoder.named_modules()
            if isinstance(module, torch.nn.modules.batchnorm._BatchNorm) and module.training
        ]
        if trainable or image_encoder.training or training_bn:
            raise RuntimeError(
                'stage-1 freeze invariant failed: '
                f'trainable={trainable}, image_encoder.training={image_encoder.training}, '
                f'training_bn={training_bn}'
            )
    ...
    def set_logging(self, path_cfg, is_print_where=True):
        ...
        if self.split_summary.get('enabled', False):
            with open(os.path.join(self.path_log, 'split_contract_runtime.json'), 'w') as file_obj:
                json.dump(self.split_summary, file_obj, indent=2, sort_keys=True)

        self._snapshot_run_provenance(path_cfg)

        # Full Python-source snapshot and manifest are written above.

    @staticmethod
    def _sha256_file(path_file):
    ...
    def _snapshot_run_provenance(self, path_cfg):
        """Copy executable Python sources and record hashes/environment for the run."""
        project_root = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        snapshot_root = os.path.join(self.path_log, 'source_snapshot')
        os.makedirs(snapshot_root, exist_ok=False)
        source_roots = ('datasets', 'models', 'pipelines', 'utils', 'tools')
        source_files = []
        for root_name in source_roots:
            ...
            for current_root, dir_names, file_names in os.walk(root_path):
                ...
                for file_name in sorted(file_names):
                    ...
                    destination = os.path.join(snapshot_root, relative_path)
        ...
        for file_name in ('main_train_0.py', 'main_cond_0.py'):
            source_path = os.path.join(project_root, file_name)
            if not os.path.isfile(source_path):
                continue
            destination = os.path.join(snapshot_root, file_name)
            shutil.copy2(source_path, destination)
            source_files.append({
                'path': file_name,
                'sha256': self._sha256_file(destination),
        ...
        config_path = os.path.realpath(path_cfg)
        stage1_path = os.path.realpath(str(self.cfg.MODEL.IMG_CLS.MODEL_PATH))
        provenance = {
            'project_root': project_root,
            'command': list(sys.argv),
            'working_directory': os.getcwd(),
            'config': config_path,
            'config_sha256': self._sha256_file(config_path),
            'stage1_checkpoint': stage1_path,
            'stage1_checkpoint_sha256': self._sha256_file(stage1_path),
            'stage1_lineage': getattr(self, 'split_summary', {}).get('stage1', {'formal_holdout_certified': False}),
            ...
            'environment': {
                ...
                'open3d': getattr(o3d, '__version__', None),
            },
        }
        with open(os.path.join(self.path_log, 'run_provenance.json'), 'w') as file_obj:
            json.dump(provenance, file_obj, indent=2, sort_keys=True)
        ...
        for epoch in range(epoch_start, epoch_end):
            ...
            for idx_iter, dict_datum in enumerate(pbar):
                ...
                if torch.isfinite(loss):
                    ...
                    raise FloatingPointError(f"non-finite training loss; meta={dict_datum['meta']}")
            ...
            if self.is_validate:
                ...
                if self.is_consider_subset:
                    if ((epoch + 1) % self.val_per_epoch_subset) == 0:
                        if self.use_clean_evaluator:
                            raise ValueError('clean model selection forbids subset validation')
                ...
                if run_standard_full_eval:
                    if self.skip_standard_full_when_conditional and self.run_conditional_at_end:
                        ...
                        if self.use_clean_evaluator:
                            evaluation_results = self.validate_kitti_clean(
                                epoch,
                                list_conf_thr=self.list_val_conf_thr,
                                eval_split=self.eval_split_during_training,
                                conditional=False,
                            )
                            self._save_best_stage2(epoch, evaluation_results)
                        else:
    ...
    @staticmethod
    def _write_kitti_lines(path_file, lines):
        with open(path_file, 'w') as file_obj:
            for line in lines:
                file_obj.write(line.rstrip('\n') + '\n')

    def _clean_eval_dataset(self, eval_split):
        if eval_split == 'val':
            if self.dataset_val is None:
                raise RuntimeError('clean validation requested but DATASET.PATH_SPLIT.val is not configured')
            return self.dataset_val
        if eval_split == 'test':
            return self.dataset_test
        raise ValueError(f'unsupported clean evaluation split: {eval_split!r}')

    def _save_best_stage2(self, epoch, evaluation_results):
        conf_key = str(float(self.list_val_conf_thr[0]))
        group_results = evaluation_results['metrics'][conf_key]['all']
        class_metrics = next(
            (item for item in group_results if item['cls'] == self.model_selection_class), None
        )
        if class_metrics is None:
            raise KeyError(f'model-selection class {self.model_selection_class!r} not found')
        iou_index = min(
            range(len(class_metrics['iou'])),
            key=lambda index: abs(class_metrics['iou'][index] - self.model_selection_iou),
        )
        if abs(class_metrics['iou'][iou_index] - self.model_selection_iou) > 1e-6:
            raise KeyError(f'model-selection IoU {self.model_selection_iou} not found')
        score = float(class_metrics['3d'][iou_index])
        if not math.isfinite(score):
            raise RuntimeError(f'non-finite internal-val model-selection score: {score}')
        print(
            f'* Stage-2 internal-val selection: epoch={epoch}, '
            f'{self.model_selection_class} AP3D_R11@{self.model_selection_iou:.1f}={score:.6f}'
        )
        if score <= self.best_val_score:
            return False
        self.best_val_score = score
        if not self.is_logging:
            raise RuntimeError('stage-2 best-checkpoint selection requires logging to be enabled')
        model_to_save = self.network.module if self.is_distributed else self.network
        model_path = os.path.join(self.path_log, 'models', 'best_val.pt')
        util_path = os.path.join(self.path_log, 'utils', 'best_val.pt')
        torch.save(model_to_save.state_dict(), model_path)
        payload = {
            'epoch': epoch,
            'selection_split': self.eval_split_during_training,
            'selection_class': self.model_selection_class,
            'selection_iou': self.model_selection_iou,
            'selection_metric': 'AP3D_R11',
        ...
        selection_metadata = {
            ...
            'optimizer_state_path': util_path,
        }
        with open(os.path.join(self.path_log, 'best_val_metrics.json'), 'w') as file_obj:
            json.dump(
                {'selection': selection_metadata, 'evaluation': evaluation_results},
                file_obj,
                indent=2,
                sort_keys=True,
            )
        print(f'* New best stage-2 internal-val checkpoint: {model_path}')
        return True

    def validate_kitti_clean(
        self,
        epoch=None,
        list_conf_thr=None,
        eval_split='val',
        conditional=False,
        resume_from=0,
        resume_path_dir=None,
    ):
        """Strict all-sample evaluator for internal validation or one-shot official testing."""
        if self.is_distributed and self.local_rank != 0:
            return None
        if self.ap_mode != 'R11':
            raise ValueError(f'unsupported AP mode: {self.ap_mode}')
        if list_conf_thr is None:
            list_conf_thr = self.list_val_conf_thr

        dataset_eval = self._clean_eval_dataset(eval_split)
        model_to_eval = self.network.module if self.is_distributed else self.network
        model_to_eval.eval()
        resume_from = int(resume_from)
        if resume_from < 0 or resume_from > len(dataset_eval):
            raise ValueError(f'invalid clean-evaluation resume index: {resume_from}')
        is_resume = resume_from > 0
        loader_dataset = (
            Subset(dataset_eval, range(resume_from, len(dataset_eval)))
            if is_resume else dataset_eval
        )
        ...
        epoch_name = 'none' if epoch is None else f'epoch_{epoch}'
        path_dir = (
            os.path.realpath(str(resume_path_dir))
            if resume_path_dir is not None
            else os.path.join(self.path_log, 'eval_kitti', eval_split, epoch_name)
        )
        if resume_path_dir is not None and not is_resume:
            raise ValueError('resume_path_dir requires resume_from > 0')
        known_groups = ['all']
        if conditional:
            known_groups += [
                'urban', 'highway', 'countryside', 'alleyway', 'parkinglots', 'shoulder',
                'mountain', 'university', 'day', 'night', 'normal', 'overcast', 'fog',
                'rain', 'sleet', 'lightsnow', 'heavysnow',
            ]

        for conf_thr in list_conf_thr:
            for group_name in known_groups:
                group_dir = os.path.join(path_dir, str(float(conf_thr)), group_name)
                if is_resume:
                    for required_name in ('pred', 'gt', 'desc', 'val.txt'):
                        required_path = os.path.join(group_dir, required_name)
                        if not os.path.exists(required_path):
                            raise FileNotFoundError(
                                f'missing clean-evaluation prefix artifact: {required_path}'
                            )
                else:
                    os.makedirs(os.path.join(group_dir, 'pred'), exist_ok=True)
                    os.makedirs(os.path.join(group_dir, 'gt'), exist_ok=True)
                    os.makedirs(os.path.join(group_dir, 'desc'), exist_ok=True)
                    with open(os.path.join(group_dir, 'val.txt'), 'w') as file_obj:
                        file_obj.write('')

        failure_count = 0
        with torch.no_grad():
            progress = tqdm(data_loader, desc=f'* CleanEval ({eval_split})')
            for sample_index, dict_datum in enumerate(progress, start=resume_from):
                dict_datum['idx_iter'] = sample_index
                dict_datum['local_rank'] = self.local_rank
                try:
                    dict_out = model_to_eval(dict_datum)
                    inference_ok = True
                except Exception as exc:
                    failure_count += 1
                    print(f'* CleanEval failure split={eval_split} index={sample_index}: {exc}')
                    print(f"* Meta info: {dict_datum.get('meta', None)}")
                    traceback.print_exc()
                ...
                description = dict_out['meta'][0]['desc']
                sample_groups = ['all']
                if conditional:
                    sample_groups += [
                        description['road_type'], description['capture_time'], description['climate']
                    ]

                for conf_thr in list_conf_thr:
                    if inference_ok:
                        current = model_to_eval.list_modules[-1].get_nms_pred_boxes_for_single_sample(
                            dict_out, conf_thr, is_nms=True
                        )
                    else:
                        current = update_dict_feat_not_inferenced(dict_out)
                    if current is None:
                        raise RuntimeError(f'post-processing returned None for {eval_split}[{sample_index}]')
                    current = dict_datum_to_kitti(self, current)

                    for group_name in sample_groups:
                        group_dir = os.path.join(path_dir, str(float(conf_thr)), group_name)
                        if is_resume:
                            for artifact_name in ('gt', 'pred', 'desc'):
                                artifact_path = os.path.join(
                                    group_dir, artifact_name, sample_name + '.txt'
                                )
                                if os.path.exists(artifact_path):
                                    raise FileExistsError(
                                        f'refusing to overwrite prefix artifact: {artifact_path}'
                                    )
                        self._write_kitti_lines(
                            os.path.join(group_dir, 'gt', sample_name + '.txt'), current['kitti_gt']
                        )
                        self._write_kitti_lines(
                            os.path.join(group_dir, 'pred', sample_name + '.txt'), current['kitti_pred']
                        )
                        with open(os.path.join(group_dir, 'desc', sample_name + '.txt'), 'w') as file_obj:
                            file_obj.write(current['kitti_desc'])
                        with open(os.path.join(group_dir, 'val.txt'), 'a') as file_obj:
                            file_obj.write(sample_name + '\n')

        if failure_count != 0 and self.fail_on_inference_error:
            raise RuntimeError(f'clean evaluation had {failure_count} inference failures')
        ...
        evaluation_results = {
            ...
            'protocol': {
                'ap_mode': 'R11',
                'confidence_thresholds': [float(value) for value in list_conf_thr],
                'nms_iou_threshold': float(model_to_eval.head.nms_thr),
                'empty_gt_policy': 'include',
                'inference_error_policy': 'raise' if self.fail_on_inference_error else 'count_as_empty',
                'conditional_breakdown': bool(conditional),
            },
            'metrics': {},
        }
        if is_resume:
            evaluation_results['resume'] = {
                'resumed': True,
                'prefix_samples': resume_from,
                'new_samples': len(dataset_eval) - resume_from,
                'resume_path_dir': path_dir,
            }
        log_step = 0 if epoch is None else epoch
        for conf_thr in list_conf_thr:
            conf_key = str(float(conf_thr))
            evaluation_results['metrics'][conf_key] = {}
            for group_name in known_groups:
                group_dir = os.path.join(path_dir, conf_key, group_name)
                split_path = os.path.join(group_dir, 'val.txt')
                val_ids = read_imageset_file(split_path)
                if not val_ids:
                    continue
                gt_annos = kitti.get_label_annos(os.path.join(group_dir, 'gt'), val_ids)
                dt_annos = kitti.get_label_annos(os.path.join(group_dir, 'pred'), val_ids)
                group_metrics = []
                for class_index in self.list_val_care_idx:
                    raw_metrics, result_log = get_official_eval_result(
                        gt_annos, dt_annos, class_index, is_return_with_dict=True
                    )
                    metrics = {
                        'cls': raw_metrics['cls'],
                        'iou': [float(value) for value in raw_metrics['iou']],
                        'bev': [float(value) for value in raw_metrics['bev']],
                        '3d': [float(value) for value in raw_metrics['3d']],
                    }
                    group_metrics.append(metrics)
                    print(f'-----split={eval_split} group={group_name} conf={conf_key} AP_R11-----')
                    print(result_log)
                    if self.is_logging:
                        for metric_name in ('bev', '3d'):
                            for iou, value in zip(metrics['iou'], metrics[metric_name]):
                                self.log_test.add_scalar(
                                    f'{eval_split}/{group_name}/{metric_name}_iou_{iou}_conf_{conf_key}',
                                    value,
                                    log_step,
                                )
                evaluation_results['metrics'][conf_key][group_name] = group_metrics

        protocol_path = os.path.join(path_dir, 'evaluation_protocol_and_results.json')
        with open(protocol_path, 'w') as file_obj:
            json.dump(evaluation_results, file_obj, indent=2, sort_keys=True)
        print(f'* CleanEval complete: split={eval_split}, samples={len(dataset_eval)}, failures={failure_count}')
        print(f'* CleanEval artifact: {protocol_path}')
        return evaluation_results

    # V2 (legacy evaluator retained only for reproducing pre-audit runs)
