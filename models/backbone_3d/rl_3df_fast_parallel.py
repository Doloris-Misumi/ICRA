...
Exact sklearn KNN with eight query workers and unchanged backend selection.
...
def exact_knn_legacy_boundary(radar, lidar, k):
    ...
    # Keep sklearn's exact backend and tie handling; parallelize query batches.
    return NearestNeighbors(n_neighbors=k, n_jobs=8).fit(radar).kneighbors(
        lidar, return_distance=False)


class RL3DFBackbone_BranchingFastParallel(RL3DFBackbone_Branching):
