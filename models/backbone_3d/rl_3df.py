...
class RL3DFBackbone_Branching(RL3DFBackbone_knngate):
    def __init__(self, cfg):
        ...
        self.image_feature_dim = int(cfg.MODEL.BACKBONE.get('IMAGE_FEATURE_DIM', 512))
        if self.image_feature_dim not in (64, 512):
            raise ValueError('IMAGE_FEATURE_DIM must be 64 (3D-LRF) or 512 (legacy WCBR)')
        self.image_condition_proj = nn.Linear(64, 512) if self.image_feature_dim == 64 else nn.Identity()
        self.routing_mode = str(cfg.MODEL.BACKBONE.get('ROUTING_MODE', 'dynamic'))
        if self.routing_mode not in ('dynamic', 'uniform', 'static'):
            raise ValueError('ROUTING_MODE must be dynamic, uniform, or static')
        if self.routing_mode == 'static':
            self.static_branch_logits = nn.Parameter(torch.zeros(3))
        self.condition_mode = str(cfg.MODEL.BACKBONE.get('CONDITION_MODE', 'camera_sensor'))
        if self.condition_mode not in ('camera_sensor', 'sensor_only', 'camera_only'):
    ...
    def _image_condition(self, data):
        if self.image_feature_dim == 64:
            feature = data['img_cls_gap'].squeeze(-1)
        else:
            feature = data['img_embedding']
        if feature.ndim != 2 or feature.shape[1] != self.image_feature_dim:
            raise ValueError('camera condition shape does not match IMAGE_FEATURE_DIM')
        token = self.image_condition_proj(feature)
        # A conditioning ablation, not a claim that camera computation is skipped.
        return torch.zeros_like(token) if self.condition_mode == 'sensor_only' else token

    def _route(self, condition_token):
        if self.routing_mode == 'dynamic':
            logits = self.branch_router(condition_token)
        elif self.routing_mode == 'static':
            logits = self.static_branch_logits.unsqueeze(0).expand(condition_token.shape[0], -1)
        else:
            logits = condition_token.new_zeros(condition_token.shape[0], 3)
        weights = torch.softmax(logits, dim=-1)
        return (1.0 - 3.0 * self.branch_weight_floor) * weights + self.branch_weight_floor
