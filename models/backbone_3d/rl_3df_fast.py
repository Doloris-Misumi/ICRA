...
Exact Euclidean KNN via cKDTree. Boundary ties use original sklearn selection.
...
def exact_knn_legacy_boundary(radar, lidar, k):
    ...
    tree = cKDTree(radar)
    count = min(k + 1, len(radar))
    distances, indices = tree.query(lidar, k=count, eps=0.0, workers=1)
    distances = np.asarray(distances).reshape(len(lidar), count)
    indices = np.asarray(indices).reshape(len(lidar), count)
    result = indices[:, :k].copy()
    if k < len(radar):
        tied = distances[:, k-1] == distances[:, k]
        if tied.any():
            # Regular voxel grids often tie. Preserve the old boundary selection,
            # rather than silently substitute different radar features.
            reference = NearestNeighbors(n_neighbors=k).fit(radar)
            result[tied] = reference.kneighbors(lidar[tied], return_distance=False)
    return result


class RL3DFBackbone_BranchingFast(RL3DFBackbone_Branching):
        ...
        for idx_layer in range(self.num_layer):
            ...
            if xL_fused is not None and xR is not None:
                ...
                for batch_idx in range(batch):
                    ...
                    value = getattr(self, f'value_layer{idx_layer}')(radar_feat_b)[knn_indices]
