...
class RadarSparseProcessor(nn.Module):
    def __init__(self, cfg):
        ...
        self.spatial_shape = [
            int((z_max-z_min) / self.grid_size),
            int((y_max-y_min) / self.grid_size),
            int((x_max-x_min) / self.grid_size),
        ]
    ...
    def forward(self, dict_item):
        if self.type_data == 0:
            ...
            # Every index passed to spconv must be in range, including batches
            # containing a mixture of valid and invalid points.
            spatial = torch.as_tensor(
                self.spatial_shape, device=sp_indices_.device, dtype=sp_indices_.dtype
            )
            valid = torch.all(sp_indices_[:, 1:] >= 0, dim=1)
            valid &= torch.all(sp_indices_[:, 1:] < spatial, dim=1)
            sp_cube = sp_cube[valid]
            sp_indices_ = sp_indices_[valid]
