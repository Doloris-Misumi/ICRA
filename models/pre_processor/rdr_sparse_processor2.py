...
class RadarSparseProcessor_L(nn.Module):
    def __init__(self, cfg):
        ...
        self.input_dim = cfg.MODEL.PRE_PROCESSOR2.INPUT_DIM
        self.spatial_shape = [
    ...
    def forward(self, dict_item):
        if self.type_data == 0:
            ...
            spatial = torch.as_tensor(
                self.spatial_shape, device=sp_indices.device, dtype=sp_indices.dtype
            )
            valid = torch.all(sp_indices[:, 1:] >= 0, dim=1)
            valid &= torch.all(sp_indices[:, 1:] < spatial, dim=1)
            sp_cube = sp_cube[valid]
            sp_indices = sp_indices[valid]
