...
def build_optimizer(p_pline, model):
    ...
    ###########################################################################
    img_cls_cfg = p_pline.cfg.MODEL.get('IMG_CLS', None)
    if img_cls_cfg is not None and hasattr(model, 'img_cls'):
        img_cls_model_path = img_cls_cfg.MODEL_PATH
        ...
        model.img_cls.load_state_dict(state_dict, strict=True)
    ###########################################################################
    ...
    model.img_cls.eval()
    if any(param.requires_grad for param in model.img_cls.parameters()):
        raise RuntimeError('stage-1 image classifier must be completely frozen in stage 2')
...
def build_scheduler(p_pline, optimizer):
    ...
    # DistributedSampler pads each rank to ceil(N/world_size); loader drops its
    # final partial batch. Count optimizer steps per rank, not global batches.
    world_size = int(getattr(p_pline, 'world_size', 1)) if getattr(p_pline, 'is_distributed', False) else 1
    steps_per_epoch = math.ceil(p_pline.cfg.DATASET.NUM / world_size) // batch_size
    schedule_epochs = int(p_pline.cfg.OPTIMIZER.get('SCHEDULE_EPOCHS', max_epoch))
    if steps_per_epoch < 1 or schedule_epochs < max_epoch:
        raise ValueError('scheduler requires nonempty epochs and SCHEDULE_EPOCHS >= MAX_EPOCH')
    if type_total_iter == 'every':
        total_iter = steps_per_epoch
    elif type_total_iter == 'all':
        total_iter = steps_per_epoch * schedule_epochs
    else:
        raise ValueError('TYPE_TOTAL_ITER must be every or all')
