...
class KRadarDetection_v1_0(Dataset):
    ...
    def should_emit_condition_prompt(self):
        try:
            prompt_cfg = self.cfg.MODEL.get('CONDITION_PROMPT', {})
        except Exception:
            prompt_cfg = {}
        if self.split == 'train':
            return bool(prompt_cfg.get('USE_GT_PROMPT_IN_TRAIN', False))
        return bool(prompt_cfg.get('ALLOW_GT_PROMPT_AT_TEST', False))
    ...
    * We push vis codes without sp cube into util_dataset for readability
    ...
    def collate_fn(self, list_dict_item):
        ...
        * list_dict_batch = list of item (__getitem__)
        ...
        dict_batch['label'] = []
        dict_batch['num_objs'] = []
        emit_condition_prompts = self.should_emit_condition_prompt()
        if emit_condition_prompts:
            dict_batch['condition_prompts'] = [] # Optional oracle/contrastive prompt list
